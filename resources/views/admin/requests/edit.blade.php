...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-2">Editar Requisição #{{ $request->id }}</h1>
        <p class="text-gray-600">Atualize os dados da requisição.</p>
    </div>

    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('requests.update', $request) }}" method="POST" id="requestForm">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="requested_at" class="block text-gray-700 text-sm font-bold mb-2">Data da Requisição:</label>
                <input type="date" name="requested_at" id="requested_at" value="{{ old('requested_at', \Carbon\Carbon::parse($request->requested_at)->format('Y-m-d')) }}" 
            ...
            <div class="mb-4">
                <p class="block text-gray-700 text-sm font-bold mb-2">Usuário: {{ $request->user->name }}</p>
            </div>
                    ...
                    @foreach($request->items as $index => $item)
                        <div class="item-row bg-gray-50 p-4 rounded mb-2 flex flex-wrap items-end">
                            <div class="w-full md:w-1/2 pr-2 mb-2 md:mb-0">
                                ...
                                <select name="items[{{ $index }}][product_id]" class="product-select shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            ...
                            <div class="w-full md:w-1/4 pr-2 mb-2 md:mb-0">
                                ...
                                <input type="number" name="items[{{ $index }}][quantity]" min="1" value="{{ $item->quantity }}" class="quantity-input shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Configurar evento de remoção para os itens existentes
